...
4.两者之间的区别: 1. apply 运行前要对作为参数的数组进行一系列检验和深拷贝，而 call 则没有,apply的速度快一些
                 2. apply方法通过遍历的方法将array转换成arguments的。传递的是数组
```
# 垃圾回收机制
```js
...
1. 闭包使用的场景
  ...
  4. 封装私有变量
  5. `函数柯里化`
  将接收多个参数的函数，转换成接收单个的函数，并且返回剩余参数的执行结果。
     优点：参数复用、提前确认、延迟执行（bind方法）
...
着这个原型本身有一个内部指针指向另一个原型，相应地另一个原型也有一个指针指向另一个构造函数。这样就在实例和原型之间构造了一条原型链。
```
# 观察者模式，和发布订阅模式，有什么区别？
```js
...
`Cookie机制`是通过检查客户身上的“通行证”来确定客户身份
  ...
  解决的问题：cookie存储session的id信息，服务器根据返回的数据进行查找。
  1. 组成 Manager 、 Provider 和 Session  Session->Manager->Provider(散列表)
  `Session与cookie的区别`
...
2. GET用于获取信息，是无副作用的，是幂等的，且可缓存， 而POST 用于修改服务器上的数据，有副作用，非幂等，不可缓存。
3. 安全性：post比get更安全
4. get请求比post请求快
5. get会有缓存，而post不会
