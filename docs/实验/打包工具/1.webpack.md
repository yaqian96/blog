...
# 打包工具
```js
grunt ， gulp，webpack， rollup，parcel(零配置版webpack)
1. Grunt
...
1. 优化打包速度
   1. 缩小文件的范围
    配置alias、配置includes exclude减少webpack loader的搜索、配置noParse
   2. 使用HappyPack开启多进程Loader转换
   3. thread-loader
   把loader放置在一个worker池里面运行，以达到多线程构建
   4. 抽离第三方模块
      内置的DllPlugin DllRefercePlugin
   5. 配置缓存 cache-loader
   6. 提取公共代码
...
2. 优化打包体积
   ...
   2. externals
   引用一个库，但是又不想让webpack打包，并且又不能影响在程序中以CMD、AMD或者window/global全局等方式进行使用
   3. tree-shaking和Scope Hoisting来剔除多余代码
   4. 按需加载
```
