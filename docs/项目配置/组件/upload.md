...
const getGeekSign = async () => {
  ...
  const getGKSignature = async () => {
    const result = await get('/service-src/app/getSignature', {
      appName,
      appKey,
...
async function getSignature() {
  ...
  const Authorization = getToken()
  const result = await get('/service-src/app/getVodSignature', {}, { Authorization, sign })
  return result.data || ''
}
...
const getAuthorization = async (options, callback) => {
  ...
  const { Key } = options
  const { sign } = uploadConfig
  const resut = await get(`/service-src/app/getPutSignature`, { key: `/${Key}`, method: 'PUT' }, { Authorization, sign })
  callback({
...
const uploadVod = (params, headers = {}, timeout) => {
  const Authorization = getToken()
  return post('/service-src/upload/putVod', params, { Authorization, ...headers, timeout })
}

const putCos = (params, headers = {}, timeout) => {
  const Authorization = getToken()
  return post('/service-src/upload/putCos', params, { Authorization, ...headers, timeout })
}

// 上传视频文件
const uploadVideoFile = async ({ opBy, file, cb, headers = {}, asynchronous = true, timeout }) => {
  const { uid, name, size } = file
  const format = file.name.split('.')[file.name.split('.').length - 1] || file.type
  // 上传前等待
  ...
  const cancel = (isTimeout = false) => {
    ...
    uploader.cancel()
    const params = {
      opBy, file, headers, cb
    }
    const retryInfo = handleUploadRetry(params, 'task cancel', isTimeout ? 'timeout' : 'canceled')
    cb && cb({ name, uid, format, size, ...retryInfo })
  }

  uploader.on('media_progress', (info) => {
    !isCanced && cb && cb({ ...info, uploader: { cancel }, name, uid, size, format, status: 'uploading', error: null })
  })

  try {
    const vodRes = await uploader.done()
    const { fileId } = vodRes
    !isCanced && cb && cb({ uid, name, size, format, status: 'processing', uploader: { cancel } })
...
const uploadCosFile = async ({ opBy, file, cb, headers = {}, timeout }) => {
  ...
  const { size, uid } = file
  cb && cb({ name: fileName, uid, format, size, status: 'waiting', error: null })
  let taskId = ''
  const isCanceled = false
  const cancel = (isTimeout = false) => { // 取消上传
    taskId && cos.cancelTask(taskId)
    const params = { opBy, file, cb, headers }
    const retryInfo = handleUploadRetry(params, isTimeout ? 'timeout' : 'task canceled', 'canceled')
  ...
  cos.putObject({
    ...
    Region: uploadConfig.region || 'ap-chengdu', /* 存储桶所在地域，必须字段 */
    ...
    onProgress(progressData) {
      ...
      customTimeout()
      !isCanceled && cb && cb({ ...progressData, uploader, name: fileName, uid, format, size, status: 'uploading', error: null })
    },
    onTaskReady(id) {
      taskId = id
    }
    ...
    if (!err) {
      ...
      !isCanceled && cb && cb({ url, name: fileName, size, uid, format, status: 'processing', error: null })
      const { data } = await putCos({ fileName, format, url, size, opBy, name, remarks },
        { sign: uploadConfig.sign, ...headers }, timeout) || {}
      !isCanceled && cb && cb({ url, name: fileName, size, uid, format, resourceId: data, status: 'done', error: null })
    } else {
      const params = { opBy, file, cb, headers }
...
//
const uploadFile = async ({ opBy, file, cb, headers = {}, asynchronous = true, timeout = 60000 }) => {
  if (!(uploadConfig.appKey && uploadConfig.appName)) {
    throw new Error("在使用Geek-Upload组件时，请配置 appName， appKey. Vue.use(GeekUpload, { appName: '', appKey: '' })")
  }
