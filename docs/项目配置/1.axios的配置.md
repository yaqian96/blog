...
export function axiosInit() {
  ...
  // 拦截白名单
  const except = ['https://vod2.qcloud.com', '/service-question/exam/grade/template/download']
  // 添加响应拦截器
  http.responseInterceptor(
    function (response) {
      ...
      if (isExcept) {
        return response
      } else if (Number(response.code) === 0 || Number(response.data.code) === 0) {
    ...
    function (error) {
      ...
      if (err) {
        ...
        if (error.message === 'abort') {
          ...
          Message({ type: 'error', message: '未知错误，请联系管理员', showClose: true })
        }
      }
